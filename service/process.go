...
func (this *service) processIncoming(msg message.Message) error {
	...
	if err != nil {
		this.logger.Debugf("(%s) Error processing acked message: %v", this.cid(), err)
	}
...
func (this *service) processAcked(ackq *sessions.Ackqueue) {
	for _, ackmsg := range ackq.Acked() {
		...
		if err != nil {
			this.logger.Errorf("process/processAcked: Unable to creating new %s message: %v", ackmsg.Mtype, err)
			continue
		}

		if _, err := msg.Decode(ackmsg.Msgbuf); err != nil {
			this.logger.Errorf("process/processAcked: Unable to decode %s message: %v", ackmsg.Mtype, err)
			continue
		}

		ack, err := ackmsg.State.New()
		if err != nil {
			this.logger.Errorf("process/processAcked: Unable to creating new %s message: %v", ackmsg.State, err)
			continue
		}

		if _, err := ack.Decode(ackmsg.Ackbuf); err != nil {
			this.logger.Errorf("process/processAcked: Unable to decode %s message: %v", ackmsg.State, err)
			continue
		}

		//this.logger.Debugf("(%s) Processing acked message: %v", this.cid(), ack)
		...
		case message.PUBREL:
			...
			if err = this.onPublish(msg.(*message.PublishMessage)); err != nil {
				this.logger.Errorf("(%s) Error processing ack'ed %s message: %v", this.cid(), ackmsg.Mtype, err)
			}

		case message.PUBACK, message.PUBCOMP, message.SUBACK, message.UNSUBACK, message.PINGRESP:
			this.logger.Debugf("process/processAcked: %s", ack)
		...
		default:
			this.logger.Errorf("(%s) Invalid ack message type %s.", this.cid(), ackmsg.State)
			continue
		}
		...
		if ackmsg.OnComplete != nil {
			...
			if !ok {
				this.logger.Errorf("process/processAcked: Error type asserting onComplete function: %v", reflect.TypeOf(ackmsg.OnComplete))
			} else if onComplete != nil {
				if err := onComplete(msg, ack, nil); err != nil {
					this.logger.Errorf("process/processAcked: Error running onComplete(): %v", err)
				}
			}
		}
	}
...
func (this *service) processSubscribe(msg *message.SubscribeMessage) error {
	...
	for i, t := range topics {
		...
		this.logger.Debugf("(%s) topic = %s, retained count = %d", this.cid(), string(t), len(this.rmsgs))
	}
	...
	for _, rm := range this.rmsgs {
		if err := this.publish(rm, nil); err != nil {
			this.logger.Errorf("service/processSubscribe: Error publishing retained message: %v", err)
			return err
		}
	}
